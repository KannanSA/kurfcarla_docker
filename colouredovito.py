...
#
# *** IMPORTANT ***
# This script is formatted as a PYTHON SCRIPT MODIFIER.
#
# *** HOW TO USE ***
# 1. FIRST, manually load your 'C60.xyz' file into OVITO.
# 2. From the "Add modification..." dropdown, select "Create bonds".
# 3. From the "Add modification..." dropdown again, select "Python script".
# 4. In the new modifier's settings, click "Load script" and select this file.

import ovito
import numpy as np

# This is the main function that OVITO calls for a Python Script Modifier.
def modify(frame, data):
    """
    This function is called by the Python Script Modifier. It finds rings in the
    input data and creates 'Color' properties for particles and bonds.
    """
    # The Create Bonds modifier must be applied before this script in the pipeline.
    if not data.particles.bonds:
        print("This script requires bonds to be created first. Please add a 'Create Bonds' modifier before this script in the pipeline.")
        return

    print(f"Frame {frame}: Running bond coloring script...")

    # Set a uniform color for all particles by creating a new 'Color' property.
    ...
    # Set visual appearance from within the modifier.
    data.particles.vis.radius = 0.35
    data.particles.bonds.vis.width = 0.2
    
    print(f"Frame {frame}: Bond coloring complete.")
